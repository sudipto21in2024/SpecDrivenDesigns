...
- [x] 3. Human checkpoint + seal: present the spec + contract summary per `03-spec-driven-workflow.md` (SPEC_REVIEW→SPEC_APPROVED, CONTRACT_REVIEW→CONTRACT_APPROVED); on approval set the spec front matter `status: spec_approved`, seal the architect arm (`tracker seal`), hand off architect→backend → verify: journal section exists; `git status --short` shows only the four §2 files; single docs commit `docs(LOGI-0007): architect — create shipment + list/search spec (AC-1..AC-11) + /shipments contract (steps 1-3/3)`
